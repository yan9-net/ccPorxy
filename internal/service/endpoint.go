...
func NewEndpointService(cfg *config.Config, p *proxy.Proxy, s *storage.PostgreSQLStorage) *EndpointService {
...
func (e *EndpointService) TestEndpointLight(index int) string {
	...
	return e.TestEndpointItem(&endpoints[index])
}
func (e *EndpointService) TestEndpointName(name string) string {
	endpoints := e.config.GetEndpoints()
	...
	if endpoint == nil {
		return e.testResult(false, "invalid_name", "models", fmt.Sprintf("Invalid endpoint name: %s", name))
	}
	return e.TestEndpointItem(endpoint)
}

func (e *EndpointService) TestEndpointItem(endpoint *config.Endpoint) string {
	transformer := endpoint.Transformer
	if transformer == "" {
		transformer = "claude"
	}
